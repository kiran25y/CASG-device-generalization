...
Code and analysis for a patient-disjoint, device-target benchmark for
respiratory sound classification, and for **CASG**: a training-only residual
transfer of frequency-conditioned token statistics inside an Audio Spectrogram
Transformer (AST).
...
| Mean CPD ×10⁻³ (lower = less acquisition-sensitive) | 20.4 | 19.9 | 8.0 | **7.3** |
...
consistency alone is not established. Full per-arm values: `docs/VERIFIED_NUMBERS.md`.
...
and a different patient key, through a residual AdaIN at ρ = 0.5; class and
distillation tokens bypass the transfer. Anchor, paired and transferred maps
pass through shared blocks 10–12, special-token pooling, a 768→512→256
projector and four-class/binary heads. Classification is applied to the anchor
and transferred paths; symmetric stop-gradient KL consistency between the anchor
and each other path. **Inference is one unmodified AST pass** — no simulator, no
donor, no transfer.
...
figures/       publication figures (vector)
...
python scripts/casg_ensemble.py        # seed ensembles
python scripts/casg_figures.py         # all figures, vector + 600 dpi
```

## The v2 controlled experiment

`docs/V2_RUNBOOK.md` pre-declares a follow-up that isolates the token transfer
with a branch- and loss-matched identity control:

```bash
python scripts/v2_patch.py --check && python scripts/v2_patch.py
python scripts/v2_gates.py     # seven runtime gates, all must pass
...
pediatric clinical recordings collected under IRB approval (No. 2021-0017-02)
and **are not released here**: no audio, patient manifest or per-clip prediction
is included in this repository. De-identified analysis data may be requested
from the corresponding author, subject to institutional approval.

## Reproducibility

`analysis/freeze.json` records the repository commit and the SHA-256 digest of
every source file, configuration, manifest, checkpoint and prediction file at
the configuration freeze (2026-09-07T02:37:24Z), with a statement that no
hyperparameter or analysis variant may be selected on frozen-test results after
that timestamp.
